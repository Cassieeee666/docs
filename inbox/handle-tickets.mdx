---
title: "Handle Tickets"
description: "Learn how to respond, manage, and resolve tickets efficiently with multi-channel replies and AI-assisted tools."
---

## Ticket Actions

- **Mark as Advertisement**\
  Manually classify a ticket as spam and move it to the Advertisement view.
- **Mark as Unread**\
  Re-flag a ticket for later handling and return it to the Unread list.

  <Frame>
    ![Image](/images/image-110.png)
  </Frame>

## Ticket Reply

You can reply using any available channel based on the customer’s contact information:

<Tabs>
  <Tab title="SMS">
    If customer's phone number is available

    - From: the phone number that received the call
    - To: customer’s phone number
    - Enter the SMS content to send.
  </Tab>
  <Tab title="Email ">
    If customer's email address is available

    - From: the support inbox that received the message
    - To: customer’s email
    - Set CC and BCC
    - Customize subject and email body

    <Frame>
      ![Image](/images/image-111.png)
    </Frame>
  </Tab>
  <Tab title="Livechat ">
    If the Livechat session is still open.

    - Continue the conversation directly in the chat window
    - Enter the chat content to send.
  </Tab>
</Tabs>

## AI Assistance

### AI Summary

Automatically summarize long conversations into:

- Customer sentiment
- The final solution provided
- A concise conversation overview

<Frame>
    ![Image](/images/image-112.png)
</Frame>

### AI Recommendation Reply

The AI recommendation reply tool identifies customer intent, retrieves relevant knowledge, and generates a ready-to-send response—reducing response time and back-and-forth.